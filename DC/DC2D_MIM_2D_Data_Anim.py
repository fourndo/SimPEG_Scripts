...
#plt.scatter(srcMat[:,0,0],srcMat[:,0,1],c='r')
#plt.scatter(srcMat[lineID==0,0,0],srcMat[lineID==0,0,1],c='b')
#ax3.set_xticklabels([])
...
def animate(ii):
    ...
    global ax1, ax2, fig
    ax1 = plt.subplot(2,1,1)
    ph = DC.plot_pseudoSection(DC2D_l,ax1,stype = 'pdp', colorbar=False, clim=(vmin,vmax))
    ...
    cb = fig.colorbar(ph[0],cax=cbarax, orientation="vertical", ticks=np.linspace(vmin,vmax, 3), format="$10^{%.1f}$")
    cb.set_label("App. Cond. (S/m)",size=8)
    
    ax2 = plt.subplot(2,1,2)    
    ph = DC.plot_pseudoSection(DC2D_r,ax2,stype = 'pdp', colorbar=False, clim=(vmin,vmax))
