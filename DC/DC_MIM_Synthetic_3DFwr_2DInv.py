...
#    plt.show()
    ...
    ax = fig1.add_subplot(3,1,pp+1, aspect='equal')  
    
    if pp == 1:
        pos =  ax.get_position() 
        ax.set_position([pos.x0, pos.y0+.025,  pos.width, pos.height])
    ...
    # Remove the mineralization for plotting only
    #m2D[m2D>1.25] = 0.08
    #ax.pcolor(mesh2d.vectorNx,mesh2d.vectorNy,np.log10(m2D),edgecolor="none",alpha=0.5,cmap = 'gray')
    pc = ax.pcolor(mesh2d.vectorNx,mesh2d.vectorNy,np.log10(minv),edgecolor="none",alpha=0.75, vmin=vmin, vmax=vmax)
...
#    cbar.set_label("S/m",size=12)
    ...
    if pp == 1:
        ...
        ax2.set_position([pos.x0, pos.y0+.05,  pos.width, pos.height])
        
    #ax2.pcolor(mesh2d.vectorNx,mesh2d.vectorNy,np.log10(m2D),edgecolor="none",alpha=0.5,cmap = 'gray')
    # Add the speudo section
    if pp == 0:
        dat = DC.plot_pseudoSection(survey2D,ax2,stype=stype, dtype = dtype, colorbar=False, clim=np.asarray([vmin,0]))
    else:
        dat = DC.plot_pseudoSection(survey2D,ax2,stype=stype, dtype = dtype, clim=np.asarray([vmin,0]))
#==============================================================================
#     if pp == 0:
#     
#         circle=plt.Circle((11950,145),125,color='k',fill=False, lw=3)
...
ax = fig1.add_subplot(3,1,3, aspect='equal')  
...
# Remove the mineralization for plotting only
#m2D[m2D>1.25] = 0.08
pc = ax.pcolor(mesh2d.vectorNx,mesh2d.vectorNy,np.log10(m2D),edgecolor="none",alpha=0.5, vmin=vmin, vmax=vmax, clim=(vmin,vmax))
...
pos =  ax.get_position() 
cbarax = fig1.add_axes([pos.x0+.2, pos.y0-.05,  pos.width*0.5, pos.height*0.05])  ## the parameters are the specified position you set 
cb = plt.colorbar(pc, cax=cbarax, orientation="horizontal", ax = ax, ticks=np.linspace(vmin,vmax, 4), format="$10^{%.1f}$")
