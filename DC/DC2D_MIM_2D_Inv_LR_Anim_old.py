...
#for ii in range(1):
    ...
    ax1 = plt.subplot(3,2,1)
    pp = DC.plot_pseudoSection(DC2D_l,ax1,stype ='pdp', clim = (ph[0].get_clim()[0],ph[0].get_clim()[1]), colorbar=False)
    ax1.set_title('Observed DP-P', fontsize=10)
    plt.xlim([xmin,xmax])
    plt.ylim([zmin,zmax])
    plt.gca().set_aspect('equal', adjustable='box')    
    ax1.set_xticklabels([])
    z = np.linspace(np.min(pp[2]),np.max(pp[2]), 5)
    ...
    for kk in range(2):
        ...
        if kk == 0:
            ...
            pp = DC.plot_pseudoSection(DCtemp,ax5,stype ='pdp', clim = (ph[0].get_clim()[0],ph[0].get_clim()[1]), colorbar=False)
