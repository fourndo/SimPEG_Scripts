...
#    DC2D_r.std = np.abs(np.asarray(DC2D_r.dobs))*0.05 + 2e-5
    ...
    mesh3d = Mesh.TensorMesh([hx, np.ones(1)*100. ,hz], x0=(-np.sum(padx) + np.min(srcMat[0][:,0]),id_lbe-50,np.max(srcMat[0][0,2])-np.sum(hz)))
    ...
    Mesh.TensorMesh.writeModelUBC(mesh3d,home_dir+dsep+'Model'+str(id_lbe)+'.chg',minv.T)
