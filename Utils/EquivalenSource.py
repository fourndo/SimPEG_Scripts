...
n_chunks = 1
parallelized = 'dask'
octreeObs = [10, 5]  # Octree levels below observation points
octreeTopo = [0, 0, 1, 0]
octree_levels_padding = [3, 4]
depth_core = 500
...
mesh = meshutils.mesh_builder_xyz(
        newTopo, h, padDist, mesh_type='TREE', depth_core=depth_core,
        base_mesh=meshInput,
...
if tileProblem:
    ...
    while usedRAM > maxRAM:
        ...
        # Refine on local
        meshLocal = meshutils.refine_tree_xyz(
            meshLocal, newTopo[ind_t, :], method='surface',
            octree_levels=octreeObs,
            octree_levels_padding=octree_levels_padding,
            finalize=True
        )

#            tileLayer = meshutils.activeTopoLayer(meshLocal, topo)
...
#        plt.show()
    ...
    def createLocalProb(rxLoc, wrGlobal, lims, ind):
        ...
        activeCells_t = np.ones(meshLocal.nC, dtype='bool')  # meshmeshutils.activeTopoLayer(meshLocal, topo)
