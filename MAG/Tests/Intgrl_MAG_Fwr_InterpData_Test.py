...
home_dir = 'C:\\LC\\Private\\dominiquef\\Projects\\4414_Minsim\\Modeling\\MAG\\Synthetic'
...
xr = np.linspace(-102.5, 97.5, 61)
yr = np.linspace(-52.5, 47.5, 31)
...
# Zero out
model[actv==0] = 0.
#Mesh.TensorMesh.writeModelUBC('Mesh.msh',mesh)
Mesh.TensorMesh.writeModelUBC(mesh,'Model.sus',model)
#Mesh.TensorMesh.writeModelUBC(mesh,'nullcell.dat',actv)
#actv = np.ones(mesh.nC)
#%% Forward mode ldata
...
l1_r = np.zeros((d_iter,6))
l2_r = np.zeros((d_iter,6))
linf_r = np.zeros((d_iter,6))
timer = np.zeros((d_iter,6))
...
#==============================================================================
    ...
    # Minimum curvature
    # First create linear system
    start_time = time.time()
    A = np.zeros((ndat,ndat))
    Xsub, Ysub = np.meshgrid(xsub, ysub)
    
    count = 0
    for kk in range(len(ysub)):
        
        for jj in range(len(xsub)):
            
            r = (xsub[jj] - mkvc(Xsub.T))**2. + (ysub[kk] - mkvc(Ysub.T))**2.
            A[count,:] = r.T * ( np.log( (r.T + 1e-8)**0.5 ) - 1. )
            count += 1
            
    w = sp.linalg.bicgstab(A,mkvc(dsub.T), tol=1e-6)
    ...
    for kk in range(len(xx)):
        ...
        r = (xx[kk] - mkvc(Xsub.T))**2. + (0. - mkvc(Ysub.T))**2.
        d_i2d_MinCurv[kk] = np.sum( w[0] * r.T * ( np.log( (r.T + 1e-8)**0.5 ) - 1. ))
        
    l1_r[ii,5] = np.sum( np.abs(d_line - d_i2d_MinCurv) )**0.5
    l2_r[ii,5] = np.sum( (d_line - d_i2d_MinCurv)**2. )
    linf_r[ii,5] = np.max( np.abs(d_line - d_i2d_MinCurv) )
    timer[ii,5] = (time.time() - start_time)
#==============================================================================
#     #%% FFT interpolation
