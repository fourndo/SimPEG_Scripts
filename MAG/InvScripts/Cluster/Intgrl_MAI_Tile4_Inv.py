...
#    mtemp._x0 = [x0[ii]-np.sum(padb), y0[ii]-np.sum(padb), mesh.x0[2]]
    ...
    Mesh.TensorMesh.writeUBC(mesh_t, work_dir + out_dir + tile_dirl2 + "MAI_Tile" + str(tt) + ".msh")
    Mesh.TensorMesh.writeUBC(mesh_t, work_dir + out_dir + tile_dirlp + "MAI_Tile" + str(tt) + ".msh")
...
#        meshes.append(mtemp)
    ...
    if getattr(invProb, 'l2model', None) is not None:
        Mesh.TensorMesh.writeModelUBC(mesh_t,work_dir + out_dir + tile_dirl2 + "MAI_Tile" + str(tt) + ".sus", actvMap*invProb.l2model)
    Mesh.TensorMesh.writeModelUBC(mesh_t,work_dir + out_dir + tile_dirlp + "MAI_Tile" + str(tt) + ".sus", actvMap*invProb.model)
    PF.Magnetics.writeUBCobs(work_dir+out_dir + 'MAI.pre', survey_t, invProb.dpred)
