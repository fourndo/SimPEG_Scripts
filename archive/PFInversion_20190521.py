...
if "result_folder" in list(input_dict.keys()):
    outDir = workDir + os.path.sep + input_dict["result_folder"] + os.path.sep
else
...
if input_dict["data_type"].lower() == 'ubc_grav':
    ...
    H0 = np.asarray(input_dict["inducing_field"])

    xs, ys, zs = patch.getUtmLocations(limits=sub_limits, ground=True)
    survey_GM2 = patch.createFtmgSurvey(inducing_field=H0, force_comp=component, detrend=True)

    print(survey_GM2.components, component, survey_GM2.nD)
else:
